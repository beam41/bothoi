...
type clientManager struct {
	sync.RWMutex
	gatewayClient gateway_interface.ClientVoiceConnector
	list          map[types.Snowflake]*client
}

func NewClientManager(gatewayClient gateway_interface.ClientInterface) voice_interface.ClientManagerInterface {
...
func (clm *clientManager) StartClient(guildId, channelId types.Snowflake) error {
	...
	err := clm.gatewayClient.JoinVoiceChannelMsg(guildId, channelId, sessionIdChan, voiceServerChan)
	if err != nil {
		return err
	}
...
func (clm *clientManager) StopClient(guildId types.Snowflake) error {
	...
	err = clm.gatewayClient.LeaveVoiceChannelMsg(guildId)
	if err != nil {
		return err
	}
	return nil
